...
const char * const BITCOIN_CONF_FILENAME = "bechorcoin.conf";
...
#else
    const char* pszModule = "bechorcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bechorcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bechorcoin";
#else
    // Unix
    return pathRet / ".bechorcoin";
#endif
#endif
}
